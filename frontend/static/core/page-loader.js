(function () {
  ...
  function injectStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = '' +
      '#' + LOADER_ID + '{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:99999;opacity:0;visibility:hidden;pointer-events:none;transition:opacity .22s ease,visibility .22s ease;background:radial-gradient(1200px 700px at 8% -10%, rgba(150,65,220,.22), transparent 60%),radial-gradient(900px 540px at 92% 0%, rgba(110,40,180,.18), transparent 55%),linear-gradient(165deg,#0c0615 0%,#150a23 46%,#0a0512 100%);}'+
      '#' + LOADER_ID + '.is-visible{opacity:1;visibility:visible;pointer-events:auto;}'+
      '#' + LOADER_ID + ' .loader-box{display:flex;flex-direction:column;align-items:center;gap:12px;color:#efe7ff;font:600 14px/1.2 Urbanist,Segoe UI,Roboto,Arial,sans-serif;letter-spacing:.02em;text-shadow:0 2px 14px rgba(0,0,0,.35);}'+
      '#' + LOADER_ID + ' .loader-spinner{width:54px;height:54px;border-radius:50%;border:3px solid rgba(255,255,255,.2);border-top-color:#c986ff;border-right-color:#8f3de0;animation:pageLoaderSpin .85s linear infinite;box-shadow:0 0 26px rgba(173,97,255,.34);}'+

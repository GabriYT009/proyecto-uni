...
def catalog(request):
    ...
    if search_param:
        ...
        Productos = Productos.filter(
            models.Q(nombre_producto__icontains=search_param) |
            models.Q(descripcion__icontains=search_param)
        )
...
def inventario(request):
    ...
    try:
        for p in page_obj.object_list:
            last_red = Historial_Inventario.objects.filter(producto=p, cantidad_nueva__lt=models.F('cantidad_anterior')).order_by('-fecha_ajuste').first()

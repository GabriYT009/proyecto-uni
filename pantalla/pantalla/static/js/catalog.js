...
// removed legacy blocking alert for mBuy to avoid interrupting other handlers

// If mBuy is present but other page handlers exist, avoid showing a blocking alert.
try {
    if (mBuy) {
        // fallback: if a product id is available, navigate to the comprar page;
        // otherwise do nothing and allow page-specific handlers to run.
        mBuy.addEventListener('click', function(e){
            try{
                ...
                if (id) {
                    window.location.href = '/producto/' + id + '/comprar/';
                }
            }catch(err){/* ignore */}
        });
    }
} catch(e){ console.warn('[home catalog.js] mBuy fallback listener failed', e); }
...
// removed legacy blocking alert for mBuy (duplicate); fallback listener added earlier
